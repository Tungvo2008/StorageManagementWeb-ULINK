...
CATALOG_IMAGE_DIR = Path(__file__).resolve().parents[2] / "assets" / "product-images"
CATALOG_LOGO_FALLBACK = Path(__file__).resolve().parents[2] / "assets" / "logo.png"
PUBLIC_IMAGE_PREFIX = f"{settings.API_V1_STR}/public/product-images/"
LEGACY_IMAGE_PREFIX = f"{settings.API_V1_STR}/products/image-files/"
...
def get_catalog_company(*, title: str = "", version: str = "") -> CatalogCompany:
    ...
    catalog_version = (version or settings.CATALOG_VERSION or "").strip()
    if not catalog_version:
        catalog_version = datetime.now().strftime("%B %Y")
    logo_path = _resolve_logo_path()
    return CatalogCompany(
        title=(title or settings.CATALOG_TITLE or "Wholesale Product Catalog").strip(),
        version=catalog_version,
        company_name=company_name,
        logo_url=str(logo_path) if logo_path else "",
        website=(settings.CATALOG_WEBSITE or "").strip(),
        email=(settings.CATALOG_EMAIL or settings.COMPANY_EMAIL or "").strip(),
        phone=(settings.CATALOG_PHONE or settings.COMPANY_PHONE or "").strip(),
        brand_color=(settings.CATALOG_BRAND_COLOR or "#15509B").strip(),
    )
...
def normalize_catalog_product(product: Product) -> CatalogProduct:
    ...
    badges = tuple(
        value.strip()
        for value in (product.catalog_badges or "").split(",")
        if value.strip()
    )
    case_pack = product.catalog_case_pack
    if case_pack is None and int(product.uom_multiplier or 1) > 1:
        case_pack = int(product.uom_multiplier)
    return CatalogProduct(
        ...
        catalog_short_name=(product.catalog_short_name or product.name).strip(),
        image_url=product.image_url,
        unit_size=(product.unit_size or "").strip(),
        case_pack=case_pack,
        country_of_origin=(product.country_of_origin or "").strip(),
        upc=(product.upc or "").strip(),
        wholesale_price=Decimal(product.unit_price or 0),
        currency=(product.currency or settings.DEFAULT_CURRENCY).upper(),
        availability="in_stock" if int(product.quantity_on_hand or 0) > 0 else "out_of_stock",
        badges=badges,
        sort_order=int(product.catalog_sort_order or 0),
    )
...
def build_catalog_categories(products: Iterable[Product]) -> tuple[CatalogCategory, ...]:
    ...
    for product in products:
        ...
        category_order = int(product.category.catalog_sort_order or 0) if product.category else 999999
...
def resolve_product_image_path(image_url: str | None) -> Path | None:
    ...
    if not value:
        return None
    for prefix in (PUBLIC_IMAGE_PREFIX, LEGACY_IMAGE_PREFIX):
...
def _resolve_logo_path() -> Path | None:
    configured = (settings.COMPANY_LOGO_PATH or "").strip()
    if configured:
...
def _register_fonts(pdfmetrics, TTFont) -> tuple[str, str]:  # type: ignore[no-untyped-def]
    regular_candidates = [
        Path(settings.INVOICE_PDF_FONT_PATH).expanduser() if settings.INVOICE_PDF_FONT_PATH else None,
        Path("/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"),
        Path("/System/Library/Fonts/Supplemental/Arial Unicode.ttf"),
        Path("/System/Library/Fonts/Supplemental/Arial.ttf"),
    ]
    bold_candidates = [
        Path(settings.INVOICE_PDF_FONT_BOLD_PATH).expanduser() if settings.INVOICE_PDF_FONT_BOLD_PATH else None,
